...
> **Cross-client byte-identity & JSON serialisation.** "Structurally identical" is sharpened to **byte-identical** by the conformance suite (`./conformance/`), which both clients' pipelines must pass. The one place this bites is re-serialising a `.json` file after a `json_patch`: Python (`json.dumps`) and JS (`JSON.stringify`) differ on float formatting (`1.0` vs `1`), non-ASCII escaping, and empty-container edge cases. The canonical form is the **JS form** (`JSON.stringify(obj, null, 2) + "\n"`), and **Python matches it** — not a style preference but a necessity, since a JS client loses integer-valued floats on `JSON.parse` (`1.0` becomes the Number `1` before it can re-serialise), so the only form both clients can hold is JS's. Floats are realistic data once the repo stores coding-project metadata (versions, coverage, benchmark timings), so this matters in practice. Implemented in Python as `operations.dumps_js_canonical` (`ensure_ascii=False` + integer-valued-float→int) and pinned by the `apply_json_canonical_serialisation` conformance case; the JS pipeline must match it when built. See `conformance/README.md` §3–§4.
...
- **User-controllable bot behaviour via shared config.** The skill template currently bakes the project's opinions about *how the bot should communicate* (terse, output-token-frugal, no unsolicited follow-up questions, no wordy patch recaps) into its body — alongside the structural rules about the mutation protocol, wikilinks, and the index. These two halves have different stability profiles: **structural rules** change with the runtime (versioned, migrated); **behaviour preferences** change with user taste (no migration, ideally per-user). Future shape:
  - Extract behaviour preferences into a dedicated section of the skill template, kept near the end so the bot internalises the structural rules first.
...
- **End-to-end test harness for sync / conflict / stash scenarios (both clients).** The mutation pipeline, sync, conflict detection, and the stash are unit- and conformance-tested, but the genuinely tricky behaviour — *what actually happens when two devices edit the same file, in a particular order, with particular timing* — is currently very hard to **observe in the real running clients** (the web app and the real Claude Code `/jade`). We have strong confidence the code is correct; we have almost no way to *watch it work*. Future work: build supporting machinery to drive these scenarios end-to-end against the real clients. Rough shape (details TBD):
  - A dedicated **test data repo** whose branches each encode a scenario's starting state and the "other device's" commits (e.g. `conflict-same-file`, `disjoint-edits`, `rename-vs-edit`, `multi-batch-first-conflict-onward`, `stash-then-resolve`).
  - **Orchestration** to set a scenario up, then inject the remote-side changes at controlled moments or in a controlled order relative to the local client's actions (to exercise the race windows: push-then-remote-advances, focus-sync mid-edit, the §6 post-push-bookkeeping gap, etc.).
  - **Teardown / rollback** to a known initial state so scenarios are repeatable and don't accumulate cruft (reset the test branches; clear the client's IndexedDB / local clone).
  - Likely a mix of: scripted GitHub-API commits for the "other device," a way to trigger the web app's sync-on-focus and the `/jade` pull/push deterministically, and assertions on the resulting stash entries / working tree / ops-log. Could lean on a browser-automation tool for the web side and a scripted `jadelens-apply` + `jadelens stash` sequence for `/jade`.
  - Value: turns "we believe the stash machinery works" into "here's the conflict happening and resolving, on the real app, on demand" — both as a confidence-builder for the maintainer and as a living demo of the design's hardest-won behaviour.
- **Client diagnostics log (a copyable "dump", not just a viewer).** Debugging the running clients today relies on the user describing what they saw in the UI; that bottleneck tightens as bugs get subtler. A device-local diagnostics log would let the user hand over an exact trace. Design notes:
  - **Surface it as a *dump*.** A "Diagnostics" section at the bottom of Settings (an advanced / developer affordance) with a recent-events list and — the important part on mobile — a **Copy to clipboard** (and optionally Download) button, so the user can paste a precise trace back rather than recount symptoms. The dump leads with build SHA, a **redacted** config snapshot, and a current queue-state snapshot (base SHA, queue length), then the recent events.
  - **Persist across reloads.** The interesting moments straddle a reload (a failed push, then the user reloads to retry), so an in-memory-only buffer loses exactly the part we care about. Use a **bounded ring buffer in IndexedDB** (last N events).
  - **Never log secrets or data.** No PAT, no file contents. Log *metadata* only: timestamp, event type (edit / enqueue / push / pull / stash / resolve / sync-on-focus), op types + paths, counts, commit/tree SHAs, HTTP status, error code/message, queue length, base SHA. Enough to reconstruct a sync sequence without leaking anything.
  - **Distinct from the operations log (§7.2).** The ops log is the *user-data* audit trail that lives in the repo and syncs; this is *client diagnostics* — device-local, disposable telemetry about the app's own behaviour. Keep them separate.
  - **Cheap, incremental.** A tiny `log(event)` helper called at the sync/queue/edit boundaries (the same points that already raise the user-facing toasts), plus the Settings panel. Add log points as bugs warrant.
