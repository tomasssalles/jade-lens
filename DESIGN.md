...
> **Cross-client byte-identity & JSON serialisation (decided, not yet implemented).** "Structurally identical" is sharpened to **byte-identical** by the conformance suite (`./conformance/`), which both clients' pipelines must pass. The one place this bites is re-serialising a `.json` file after a `json_patch`: Python (`json.dumps`) and JS (`JSON.stringify`) differ on float formatting (`1.0` vs `1`), non-ASCII escaping, and empty-container edge cases. **Decision:** the canonical form is the **JS form** (`JSON.stringify(obj, null, 2) + "\n"`), and **Python changes to match** — not a style preference but a necessity, since a JS client loses integer-valued floats on `JSON.parse` (`1.0` becomes the Number `1` before it can re-serialise), so the only form both clients can hold is JS's. Floats are realistic data once the repo stores coding-project metadata (versions, coverage, benchmark timings), so this matters in practice. Implementation is deferred until the JS pipeline lands; until then the suite pins the current Python form transitionally. See `conformance/README.md` §3–§4.
