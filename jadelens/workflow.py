...
def _post_apply_sidecar_propagation_pass(
    ...
    for op in operations:
        ...
        if isinstance(op, JsonPatch):
            ...
            needs_prune = False
            for patch_op in op.patch:
                if patch_op.get("op") != "remove":
                    continue
                ptr = patch_op.get("path", "")
                if not ptr.startswith("/") or "-" in ptr[1:].split("/"):
                    continue
                try:
                    sidecar_file = pointer_to_sidecar_path(op.path, ptr)
                except ValueError:
                    continue
                sidecar_base = sidecar_file[:-3]  # strip .md → dir for nested fields
                if (data_repo / sidecar_file).is_file():
                    try:
                        subprocess.run(
                            ["git", "-C", str(data_repo), "rm", "--force", "--",
                             sidecar_file],
                            capture_output=True, text=True, check=True,
                        )
                    except subprocess.CalledProcessError as e:
                        raise ApplyError(
                            f"Failed to delete sidecar {sidecar_file!r}: "
                            f"{e.stderr.strip()}"
                        ) from e
                    needs_prune = True
                if (data_repo / sidecar_base).is_dir():
                    try:
                        subprocess.run(
                            ["git", "-C", str(data_repo), "rm", "-r", "--force", "--",
                             sidecar_base],
                        ...
                        raise ApplyError(
                            f"Failed to delete sidecar subtree {sidecar_base!r}: "
                            f"{e.stderr.strip()}"
                        ) from e
                    needs_prune = True
            if needs_prune and sidecar_dir_path.is_dir():
                for d in sorted(sidecar_dir_path.rglob("*"), reverse=True):
                    if d.is_dir() and not any(d.iterdir()):
                        d.rmdir()
                if not any(sidecar_dir_path.iterdir()):
