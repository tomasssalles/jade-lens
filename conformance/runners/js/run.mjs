...
const DEFAULT_DATA_VERSION = 'v0.1.0';
...
function runCase(c) {
  ...
  if (Object.hasOwn(expect, 'error')) {
    ...
    if (!thrown) return { ok: false, msg: `expected error ${expect.error} but the batch succeeded` };
    if (!(thrown instanceof ConformanceError)) {
      return { ok: false, msg: `expected ConformanceError ${expect.error}, got ${thrown.stack ?? thrown}` };
    }
    if (thrown.code !== expect.error) {
      return { ok: false, msg: `expected error code ${expect.error}, got ${thrown.code}` };
    }
  ...
  try {
    ...
    return { ok: false, msg: `unexpected failure: ${e.code ?? ''} ${e.stack ?? e}` };
  }

  const actual = {};
  for (const [k, v] of out) {
    if (versionSeeded && k === '.jade/version' && !Object.hasOwn(expect.after, '.jade/version')) {
